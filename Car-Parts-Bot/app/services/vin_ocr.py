...
def _run_openai_vision_ocr(img_bytes: bytes, content_type: str) -> Dict[str, Any]:
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "Extract vehicle VIN/chassis number."},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Extract VIN. Return JSON with text + confidence."},
                    {
                        "type": "image_url",
    ...
    # print("📝 Raw OCR JSON payload:", payload)
    raw_text, confidence = _parse_json_response(payload)
    # print("📝 OCR extracted text:", raw_text)
    chassis = extract_vin_from_text(raw_text)
